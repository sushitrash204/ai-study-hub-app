...
const markdownStylesFor = (isUser: boolean, fillWidth: boolean) => ({
    ...
    table: {
        borderWidth: 1,
        borderColor: isUser ? '#7C3AED' : '#D1D5DB',
        borderRadius: 10,
        overflow: 'hidden',
        marginBottom: 10,
    },
    thead: {
        backgroundColor: isUser ? 'rgba(124,58,237,0.35)' : '#EEF2FF',
    },
    th: {
...
export default function ChatMarkdownMessage({ content, isUser, fillWidth = true }: ChatMarkdownMessageProps) {
    ...
    const copyCode = async (code: string, blockKey: string) => {
        try {
            ...
            Alert.alert('Lỗi', 'Không thể sao chép đoạn code lúc này.');
        }
    };

    const renderHighlightedCodeBlock = (language: string, rawCode: string, blockKey: string) => {
        const normalizedLanguage = normalizeCodeLanguage(language);
        const code = (rawCode || '[Không thể hiển thị nội dung code]').replace(/\n$/, '');
            ...
            >
                <View style={styles.codeHeader}>
                    ...
                    >
                        ...
                        <Text style={styles.copyText}>{copied ? 'Đã sao chép' : 'Copy'}</Text>
                    </TouchableOpacity>
                </View>
    ...
    const renderCodeBlock = (node: any) => {
        ...
        const code = rawCode || '[Không thể hiển thị nội dung code]';
    ...
    const renderInlineMath = (latex: string, nodeKey: string) => {
        ...
        const fallbackText = normalizeInlineMathText(latex) || '[Công thức]';
    ...
    const renderBlockMath = (latex: string, nodeKey: string) => {
        const safeLatex = normalizeMathBlockLatex(latex);
        const fallbackText = normalizeMathBlockText(latex) || '[Công thức trống]';
